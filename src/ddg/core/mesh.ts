			...
			if (!hasTwinHalfedge.get(h)) {
				...
				do {
					...
					if (nextHe) {
						...
						throw new Error("Invalid mesh: Boundary halfedge does not have a twin.");
					}
				} while (he !== h);
...
function indexElements(elementList: any[]) {
	let i = 0;
	let index: { [key: string]: number } = {}; // Specify the type of 'index' as an indexable object
